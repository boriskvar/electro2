    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <div id="aside" class="col-md-2">
          ...
          <form method="GET"
                ...
                @foreach ($categories as $cat)
                  <div class="input-checkbox">
                    <input type="checkbox"
                           ...
                           name="categories[]"
                           value="{{ $cat->id }}"
                           {{-- @if (in_array($cat->id, request('categories', []))) checked @endif> --}}
                           {{-- @if ((isset($category) && $cat->id == $category->id) || in_array($cat->id, request('categories', []))) checked @endif> --}}
                           @if (isset($category) && $cat->id == $category->id) checked @endif
            ...
            <button type="submit"
                    ...
                    style="background-color: #D10024; border-color: #D10024;">
              Apply
            </button>
              ...
              @foreach ($topSellingProducts as $product)
                <div class="product-widget">
                  <div class="product-img">
                    <img id="alttext-image"
                         src="{{ asset('storage/img/' . $product->images[0]) }}"
                         alt="{{ $product->name }}">
                  </div>
        ...
        <div id="app" class="col-md-9">
          ...
          <div class="store-filter clearfix">
            <div class="store-sort">
              <label>
                ...
                <select class="input-select" onchange="location = this.value;">
                  <option value="{{ request()->fullUrlWithQuery(['sort' => 'popular']) }}"
                          {{ request('sort') == 'popular' ? 'selected' : '' }}>Popular</option>
                  <option value="{{ request()->fullUrlWithQuery(['sort' => 'position']) }}"
                          {{ request('sort') == 'position' ? 'selected' : '' }}>Position</option>
                </select>
              </label>
              ...
              <label>
                Show:
                <select class="input-select" onchange="location = this.value;">
                  <option value="{{ request()->fullUrlWithQuery(['per_page' => 20]) }}"
          ...
          <!-- store products -->
          {{-- <pre>{{ json_encode($products->items(), JSON_PRETTY_PRINT) }}</pre>
          ...
          <products :products='@json($products->items())'
                    :categories='@json($categories)'
                    view-type="{{ request('view', 'grid') }}"></products>
